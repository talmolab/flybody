...
class ReferencePosesTask(composer.Task, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if isinstance(dataset, Text):
            try:
                ...
                raise
        self._load_reference_data(ref_path=ref_path, proto_modifier=proto_modifier, dataset=dataset)
        ...
        self._current_clip = self._loader.get_trajectory(self._dataset.ids[0], zero_out_velocities=False)
        ...
        self._walker_features = tree.map_structure(lambda x: x[0], self._clip_reference_features)
        self._walker_features_prev = tree.map_structure(lambda x: x[0], self._clip_reference_features)
        ...
        if self._ghost_offset is not None:
            self._ghost = utils.add_walker(walker, self._arena, name="ghost", ghost=True)
            self._ghost.observables.disable_all()

            if disable_props:
                self._ghost_props = []
            else:
                self._ghost_props = self._current_clip.create_props(prop_factory=self._ghost_prop_factory)
    ...
    def _strip_reference_prefix(self):
        ...
        if self._prop_prefixes:
            self._clip_reference_features["prop_positions"] = np.transpose(positions, axes=axes)
            self._clip_reference_features["prop_quaternions"] = np.transpose(quaternions, axes=axes)
    ...
    def _ghost_prop_factory(self, prop_proto, priority_friction=False):
        ...
        for geom in prop.mjcf_model.find_all("geom"):
            geom.set_attributes(contype=0, conaffinity=0, rgba=(0.5, 0.5, 0.5, 0.999))
        prop.observables.disable_all()
        return prop

    def _load_reference_data(self, ref_path, proto_modifier, dataset: types.ClipCollection):
        self._loader = loader.HDF5TrajectoryLoader(ref_path, proto_modifier=proto_modifier)
        ...
        if self._dataset.end_steps is None:
            ...
            self._all_clips = [
                ...
                for clip_id, clip_start_step in zip(self._dataset.ids, self._dataset.start_steps)
            ]
    ...
    def _add_observables(self, enabled_reference_observables):
        ...
        self._walker.observables.add_observable(
            ...
            base_observable.Generic(lambda _: self._reference_observations["walker/reference_rel_root_quat"]),
        )
        self._walker.observables.add_observable(
        ...
        self._walker.observables.add_observable("qvel", base_observable.Generic(lambda physics: physics.data.qvel))
    ...
    def _get_possible_starts(self):
        ...
        for clip_number, (start, end, weight) in enumerate(
            ...
            if self._always_init_at_clip_start:
                ...
                self._possible_starts += [(clip_number, j) for j in range(start, last_possible_start)]
                self._start_probabilities += [weight for _ in range(start, last_possible_start)]

        # normalize start probabilities
        self._start_probabilities = np.array(self._start_probabilities) / np.sum(self._start_probabilities)
    ...
    def initialize_episode_mjcf(self, random_state: np.random.RandomState):
        ...
        if not self._disable_props:
            self._props = self._current_clip.create_props(prop_factory=self._prop_factory)
            for prop in self._props:
                self._arena.add_free_entity(prop)
                prop.observables.position.enabled = True
                prop.observables.orientation.enabled = True
    ...
    def _compute_termination_error(self):
        ...
        target_bodies = self._clip_reference_features["body_positions"][self._time_step]
        error_bodies = np.mean(np.abs((target_bodies - self._walker_features["body_positions"])[self._body_idxs]))
        self._termination_error = 0.5 * self._body_error_multiplier * error_bodies + 0.5 * error_joints

        if self._props:
            target_props = self._clip_reference_features["prop_positions"][self._time_step]
            cur_props = self._walker_features["prop_positions"]
            # Separately compute prop termination error as euclidean distance.
            self._prop_termination_error = np.mean(np.linalg.norm(target_props - cur_props, axis=-1))
    ...
    def after_compile(self, physics: "mjcf.Physics", random_state: np.random.RandomState):
    ...
    def get_reference_rel_joints(self, physics: "mjcf.Physics"):
        ...
        diff = self._clip_reference_features["joints"][time_steps] - self._walker_joints
    ...
    def get_reference_rel_bodies_pos_global(self, physics: "mjcf.Physics"):
        ...
        time_steps = self._time_step + self._ref_steps
        return (self._clip_reference_features["body_positions"][time_steps] - self._walker_features["body_positions"])[
            :, self._body_idxs
        ].flatten()
    ...
    def get_reference_rel_bodies_pos_local(self, physics: "mjcf.Physics"):
        ...
        obs = self._walker.transform_vec_to_egocentric_frame(
            physics,
            (self._clip_reference_features["body_positions"][time_steps] - self._walker_features["body_positions"])[
                :, self._body_idxs
            ],
        )
        return np.concatenate([o.flatten() for o in obs])
    ...
    def get_reference_rel_root_pos_local(self, physics: "mjcf.Physics"):
        ...
        obs = self._walker.transform_vec_to_egocentric_frame(
            physics,
            (self._clip_reference_features["position"][time_steps] - self._walker_features["position"]),
        )
        return np.concatenate([o.flatten() for o in obs])
    ...
    def get_reference_props_pos_global(self, physics: "mjcf.Physics"):
        ...
        if self._props:
            return self._clip_reference_features["prop_positions"][time_steps].flatten()
        else:
            return []
    ...
    def get_reference_props_quat_global(self, physics: "mjcf.Physics"):
        ...
        if self._props:
            return self._clip_reference_features["prop_quaternions"][time_steps].flatten()
        else:
            return []
    ...
    def get_veloc_control(self, physics: "mjcf.Physics"):
        ...
        veloc_world = (
            self._walker_features["position"] - self._walker_features_prev["position"]
    ...
    def get_joints_vel_control(self, physics: "mjcf.Physics"):
        ...
        joints_curr, joints_prev = (
            self._walker_features["joints"],
            self._walker_features_prev["joints"],
        )
        return (joints_curr - joints_prev)[self._walker.mocap_to_observable_joint_order] / self._control_timestep

    def get_clip_id(self, physics: "mjcf.Physics"):
        """Observation of the clip id."""
        del physics  # physics unused by get_clip_id.

        return np.array([self._current_clip_index]).astype(float)  # modified for VNL

    def get_all_reference_observations(self, physics: "mjcf.Physics"):
        reference_observations = dict()
        reference_observations["walker/reference_rel_bodies_pos_local"] = self.get_reference_rel_bodies_pos_local(
            physics
        )
    ...
    def _update_ghost(self, physics: "mjcf.Physics"):
        if self._ghost_offset is not None:
            target = tree.map_structure(lambda x: x[self._time_step], self._clip_reference_features)
            utils.set_walker_from_features(physics, self._ghost, target, self._ghost_offset)
            if self._ghost_props:
                utils.set_props_from_features(physics, self._ghost_props, target, z_offset=self._ghost_offset)
            mjlib.mj_kinematics(physics.model.ptr, physics.data.ptr)
    ...
    def action_spec(self, physics: "mjcf.Physics"):
        ...
        return specs.BoundedArray(
            ...
            name="\t".join(
                actuator.full_identifier for actuator in self._walker.actuators
...
class MultiClipMocapTracking(ReferencePosesTask):
    ...
    def __init__(
        ...
        self._walker.observables.add_observable(
            "time_in_clip", base_observable.Generic(self.get_normalized_time_in_clip)
        )
    ...
    def after_step(self, physics: "mjcf.Physics", random_state):
        ...
        self._current_reference_features = {
            k: v[self._time_step].copy() for k, v in self._clip_reference_features.items()
        }
    ...
    def get_normalized_time_in_clip(self, physics: "mjcf.Physics"):
        ...
        normalized_time_in_clip = (self._current_start_time + physics.time()) / self._current_clip.duration
...
class PlaybackTask(ReferencePosesTask):
    ...
    def _get_clip_to_track(self, random_state: np.random.RandomState):
        ...
        self._current_clip = self._all_clips[self._current_clip_index]
        self._clip_reference_features = self._current_clip.as_dict()
        self._clip_reference_features = _strip_reference_prefix(self._clip_reference_features, "walker/")
        # The reference features are already restricted to
        # clip_start_step:clip_end_step. However start_step is in
        # [clip_start_step:clip_end_step]. Hence we subtract clip_start_step to
        # obtain a valid index for the reference features.
