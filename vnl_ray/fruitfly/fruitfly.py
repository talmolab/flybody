...
def change_body_frame(body, frame_pos, frame_quat):
    ...
    frame_quat = np.array((1.0, 0, 0, 0)) if frame_quat is None else frame_quat
    # Get frame transformation.
    body_pos = np.zeros(3) if body.pos is None else body.pos
    dpos = body_pos - frame_pos
    body_quat = np.array((1.0, 0, 0, 0)) if body.quat is None else body.quat
    ...
    for child in body.all_children():
        ...
        if hasattr(child, "quat"):
            child_quat = np.array((1.0, 0, 0, 0)) if child.quat is None else child.quat
...
class FruitFlyObservables(legacy_base.WalkerObservables):
    ...
    def vestibular(self):
        ...
        return [self.gyro, self.accelerometer, self.velocimeter, self.world_zaxis]
