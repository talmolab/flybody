...
import os
import numpy as np
from flybody.tasks.constants import _WALK_CONTROL_TIMESTEP, _WALK_PHYSICS_TIMESTEP
...
qpos[:, [2, 3]] = [0.14355, 1.0]
qvel = np.zeros((n_steps, 6))
qvel[:, 0] = 1.0  # Speed: 1 cm/s.
...
def test_can_create_env_inference_mode():
    ...
    env.task._traj_generator.set_next_trajectory(snippet["qpos"], snippet["qvel"])
    timestep = env.reset()
...
def test_can_step_env_inference_mode():
    ...
    for _ in range(100):
        ...
        assert timestep.reward == 1.0  # At test-time, reward is 1.
