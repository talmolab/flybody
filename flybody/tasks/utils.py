...
def add_walker(walker_fn, arena, name="walker", ghost=False, visible=True, position=(0, 0, 0)):
    """Create a walker."""
    ...
    if ghost:
        ...
        if skin:
            if visible:
                ...
                skin.set_attributes(rgba=(0.5, 0.5, 0.5, 0.0))
...
def set_props_from_features(physics, props, features, z_offset=0):
    ...
    if np.isscalar(z_offset):
        z_offset = np.array([0.0, 0.0, z_offset])
...
def get_features(physics, walker, props=None):
    ...
    end_effectors = np.array(walker.observables.end_effectors_pos(physics)[:]).reshape(-1, 3)
    walker_features["end_effectors"] = end_effectors
    if hasattr(walker.observables, "appendages_pos"):
        appendages = np.array(walker.observables.appendages_pos(physics)[:]).reshape(-1, 3)
    else:
