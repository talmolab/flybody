...
_XML_PATH = os.path.join(os.path.dirname(__file__), "assets_rodent/rodent.xml")
...
class Rat(legacy_base.Walker):
    ...
    def mocap_tracking_bodies(self):
        ...
        return tuple(
            body for body in self._mjcf_root.find_all("body") if not re.match(r"(vertebra|hand|toe)", body.name)
        )
