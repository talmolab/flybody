...
_MASS = {"head": 0.15, "thorax": 0.34, "abdomen": 0.38, "leg": 0.0162, "wing": 0.008}
...
def main(argv: Sequence[str]):
    ...
    for tarsus4 in tarsi4:
        ...
        pos = gclaw.pos - ipos
        gclaw.remove()
        claw.add("geom", name=name, material=material, quat=quat, mesh=mesh, pos=pos)
    ...
    for leg in legs:
        ...
        if "T2" in leg.name or "T3" in leg.name:
            ...
            tarsus_pos[2] -= 0.00175
            tarsus_quat = mat_to_quat(mat)
        else:
            if "left" in leg.name:
                rquat = np.array((1.0, 0, 0, 0))
            else:
                rquat = np.array((0.0, 1.0, 0, 0))
        ...
        while children:
            child = children[0]
            change_body_frame(child, child.pos, np.array((1.0, 0, 0, 0)))
            parent = child
            children = parent.get_children("body")
            ...
            if len(children) == 1:
                ...
                if "coxa" in geom.name:
                    ...
                    if "femur" in geom.name:
                        from_[2] = 0.004375 * (1 if "left" in geom.name else -1)
                        from_[1] = 0.002625 * (1 if "left" in geom.name else -1)
                        ...
                        fromto[1] += 0.003 * (-1 if "left" in geom.name else 1)
                        fromto[2] += 0.005 * (-1 if "left" in geom.name else 1)
                        size = (1.05 * size[0],)
                    else:
                        fromto = np.hstack((from_, children[0].pos))
                        size = (1.225 * size[0],)
    ...
    pitch_range = 2.0 / 3.0 * np.pi * np.array((-1.0, 1.0)) - _YAW_AXIS_PITCH
    ...
    print("Head joints.")
    head_joint_range = {"twist": (-3, 3), "abduct": (-0.2, 0.2), "extend": (-0.5, 0.3)}
    ...
    for haltere in halteres:
        if "left" in haltere.name:
            rotate_y = np.array((0.0, 0.0, 1.0, 0.0))
    ...
    for joint in model.find_all("joint"):
        ...
        if "twist" in joint.name or "abduct" in joint.name or "extend" in joint.name:
            ...
            model.actuator.add("general", name=joint.name, joint=joint, dclass=dclass)
    ...
    offset = physics.bind(thorax).xipos
    offset[1] = 0.0
    thorax.pos = np.array((0.0, 0.0, 0.0))
    change_body_frame(thorax, offset, np.array((1.0, 0.0, 0.0, 0.0)))
    if thorax.quat[0] == 1:
        thorax.quat = None
    thorax.pos = None
